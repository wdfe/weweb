...
TemplateExparser.parse = function (value, methods) {
    ...
    if (idx % 2) {
      ...
      if (methodSlices[3]) {
        ...
        for (let argIdx = 0; argIdx < args.length; argIdx++) {
          boundPropList.indexOf(args[argIdx]) < 0 &&
            boundPropList.push(args[argIdx])
        }
      } else {
        // single arg
        boundPropList.indexOf(methodSlices[2]) < 0 &&
  ...
  tempObj.isSingleletiable =
    slices.length === 3 && slices[0] === '' && slices[2] === '' // 仅表达式
