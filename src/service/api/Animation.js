...
class Animation {
  constructor (...options) {
    ...
    this.option = {
      transition: {
        duration:
          typeof option.duration !== 'undefined' ? option.duration : 400,
        timingFunction:
          typeof option.timingFunction !== 'undefined'
  ...
  step () {
    ...
    this.currentStepAnimates.forEach(function (animate) {
      animate.type !== 'style'
        ? (that.currentTransform[animate.type] = animate)
        : (that.currentTransform[
          animate.type + '.' + animate.args[0]
        ] = animate)
    })
    this.actions.push({
      ...
      option: {
        transformOrigin:
          typeof params.transformOrigin !== 'undefined'
            ? params.transformOrigin
            : this.option.transformOrigin,
        transition: {
          duration:
            typeof params.duration !== 'undefined'
              ? params.duration
              : this.option.transition.duration,
          timingFunction:
            typeof params.timingFunction !== 'undefined'
              ? params.timingFunction
              : this.option.transition.timingFunction,
          delay:
            typeof params.delay !== 'undefined'
              ? params.delay
              : this.option.transition.delay
        }
      }
    })
    this.currentStepAnimates = []
