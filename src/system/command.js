...
function getRoutes() {
  ...
  if (routes.indexOf(path) !== routes.length - 1) {//当前的页面不是路由表中最后一个 每次进入都可以到退出时的页面
...
export function APP_SERVICE_COMPLETE(data) { //eslint-disable-line
  ...
  if (routes.length) {
    ...
    Bus.once('ready', id => {
      ...
      let serial = new Serial()//顺序执行注册函数
      ...
      for (let route of routes) {//为路由表中的页面注册ready事件
        ...
        serial.add(cb => {
          let data = getRedirectData(`/${route}`, viewManage.currentView().id)//获取到路由数据
          toAppService(data)
          Bus.once('ready', () => cb())
        })
      }
