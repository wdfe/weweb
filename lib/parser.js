...
function parseImports (file, wxss, cb) {
  ...
  util[fn](srcs, file, function (err) {
    if (err) {
      console.error('=> ParseImports Error <=')
      return cb(err)
    }
    srcs = srcs.map(src => {
...
module.exports = function (fullPath) {
  ...
  return new Promise(function (resolve, reject) {
    if (/\.wxml$/.test(fullPath)) {
      parseImports(fullPath, false, (err, srcs) => {
        ...
        if (isLinux) {
          execWcc = exec.bind(
            null,
            [wcc].concat(wxmlArgs).concat(srcs).join(' ')
          )
        }
      ...
      parseImports(fullPath, true, (err, srcs) => {
        ...
        if (isLinux) {
          execWcsc = exec.bind(
            null,
            [wcsc].concat(wxssArgs).concat(srcs).join(' ')
          )
        }
      ...
      config().then(function (obj) {
        util.parseJavascript(obj, fullPath, config.babel).then(
          function ({ code, map }) {
    ...
    console.error('==> Parse Page Error:', e)
  })
}
