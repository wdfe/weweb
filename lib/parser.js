...
const compiler = require('./wxml-transpiler/index')
    ...
    if (/\.wxml$/.test(full_path)) {
      parseImports(full_path, false, (err, srcs) => {
        ...
        if (isLinux) {
          execWcc = exec.bind(
            null,
            [wcc].concat(wxml_args).concat(srcs).join(' ')
          )
        }
        // console.log([wcc].concat(wxml_args).concat(srcs).join(' '))
        const cpl = 1
        if (cpl == 0) {
          if (msgFlag) {
            console.log(chalk.yellow('Using wcc.exe to build: '))
            msgFlag = 0
          }
          // console.log(srcs)
        ...
        if (cpl == 1) {
          if (msgFlag) {
            console.log(chalk.yellow('Using wxml-compiler to build:'))
            msgFlag = 0
          }
          // console.log(srcs)
          const res = compiler.wxmlCompile(srcs)
          resolve(res.render)
        }
      })
      ...
      parseImports(full_path, true, (err, srcs) => {
        ...
        if (isLinux) {
          execWcsc = exec.bind(
            null,
            [wcsc].concat(wxss_args).concat(srcs).join(' ')
          )
        }
      ...
      config().then(function (obj) {
        util.parseJavascript(obj, full_path, config.babel).then(
          function ({ code, map }) {
