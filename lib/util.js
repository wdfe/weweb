...
exports.parseImports = function parseImports(res, file, cb) {//解析wxml获取页面所有引入文件路径
...
function loadJavascript(full_path, useBabel, cb) {
  if (useBabel) {
    babel.transformFile(full_path, {
      ...
      if (err) return cb(err)
      if (inProd) result.code = UglifyJS.minify(result.code, {fromString: true}).code
      cb(null, result)
    })
  } else {
  ...
  if( paths && paths.length ){
    ...
    paths.forEach(function( p ){
      ...
      if( st ){
        // 判断是否为文件
        if( st.isFile()){
          if(filter.extname && filter.extname.indexOf(path.extname(_src))!=-1){
            return
          }
          // 通过管道来传输流
