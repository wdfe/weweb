...
var escapeTxt = function (str) {
  ...
  map.forEach(function (v) { return (str = str.replace(v.ori, v.n)); });
  return str
};
...
function generate (
  ...
  var codeInfo = getCurrentCodeInfo$1();

  // generate tmplates
  var templateImpls = codeInfo.templates
    .map(function (tmpl) { return genTemplate(tmpl, state); })
    .join('');
...
function genTemplate (tmpl, state) {
  var codeInfo = getCurrentCodeInfo$1();
  var children = genElement(tmpl.tmpl, state);
...
function genElement (el, state) {
  var isTemplateWithName = el.tag == 'template' && el.name;
  if (el.staticRoot && !el.staticProcessed) {
    ...
    if (el.component) {
      ...
      if (el.tag == 'Program' || isTemplateWithName) {
        ...
        var codeInfo = getCurrentCodeInfo$1();
        var includeVector = codeInfo.ic;
        var importVector = codeInfo.ti;
        var oldIcLen = includeVector.length;
        var oldTiLen = importVector.length;
        var children = el.inlineTemplate ? null : genChildren(el, state, true);
        var newIcLen = includeVector.length;
        var newTiLen = importVector.length;
        ...
        if (dataLen == 0) {
          ...
          code = "var " + (el.nodeFuncName || 'nodeFuncName error2') + " = _n(\"" + (el.tag) + "\");\n          _r(" + (el.nodeFuncName || 'nodeFuncName error3') + ", '" + (camelize(attr.name)) + "', " + (propStore$1.map[attr.value]) + ", " + env + ", " + scope + ", gg);" + (children$1 ? ("" + children$1) : '');
        } else {
          code = "var " + (el.nodeFuncName || 'nodeFuncName error4') + " = _m( \"" + (el.tag) + "\", " + (data || 'data error') + ", " + env + ", " + scope + ", gg);" + (children$1 ? ("" + children$1) : '');
        }
      }
    }
    // module transforms
...
function genTemplateCaller (el, state) {
  // console.log(el)
  var container = el.nodeFuncName || 'name err';
  var isFunc = generateId();
  var tmplName = getCurrentCodeInfo$1().path;
  ...
  var data = dataProp ? ("_1(" + dataProp + "," + env + "," + scope + ",gg);") : "{};";
...
function genInlineTemplate (el, state) {
  ...
  if (ast.type === 1) {
    var inlineRenderFns = generate(ast, propStore$1, templateIdx, state.options);
...
function genProps (props) {
  var res = '[';
  var initIdx;
  props = props.sort(function (a, b) { return propStore$1.map[a.value] - propStore$1.map[b.value]; });
  for (var i = 0; i < props.length; i++) {
    var prop = props[i];
    if (!initIdx) {
      var idx = propStore$1.map[prop.value];
      res += "\"" + (camelize(prop.name)) + "\", " + idx + ",";
      if (idx >= 0) { initIdx = idx; }
    } else {
      res += "\"" + (camelize(prop.name)) + "\", " + (propStore$1.map[prop.value] - initIdx) + ",";
    }
  }
  return res.slice(0, -1) + ']'
}
...
// #3895, #4268
function getCurrentCodeInfo$1 () {
  return propStore$1.tmplMap[templateIdx]
}
...
var createCompiler = createCompilerCreator(function baseCompile (
  ...
  var code = program.asts
    .map(
      function (ast, idx) { return ((generate(ast.ast, program.store, idx, options).render) + "\n      e_[\"" + (ast.path) + "\"]={f:m" + idx + ",j:[],i:[],ti:[" + (program.store.tmplMap[idx].ti
